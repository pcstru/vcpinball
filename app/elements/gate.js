(function registerGate(Pin) {
    ...
    function configuredDirection(el, world) {
    ...
    function gateSwingLimits(el, restAngle, direction) {
        ...
        if (direction === "twoWay") {
            return {
                ...
                max: span,
                span: span * 2,
                oneWaySwingSign: signedSpan < 0 ? -1 : 1
            };
        }
        const swingSign = signedSpan < 0 ? -1 : 1;
        return {
            min: swingSign < 0 ? -span : 0,
            max: swingSign < 0 ? 0 : span,
            span: span,
            oneWaySwingSign: swingSign
        };
    }
    ...
    function directionAllows(el, world, pushDirection) {
        ...
        return pushDirection === limits.oneWaySwingSign;
    }

    function swingDirectionForPush(direction, pushDirection, limits) {
        if (direction === "twoWay") return pushDirection < 0 ? -1 : 1;
        return limits.oneWaySwingSign < 0 ? -1 : 1;
    }
    ...
    function forcedOpenAngle(el, world, limits) {
        const direction = configuredDirection(el, world);
        if (direction === "twoWay") return limits.max;
        if (limits.oneWaySwingSign < 0) return limits.min;
        return limits.max;
    }
    ...
    function kickGate(el, world, ball, pushDirection) {
        ...
        if (speed < 0.2) return;
        applyGateImpulse(el, state, speed, swingDirection, limits);
    ...
    Pin.elements.register("gate", {
        compile: function compile(el, table, world) {
            ...
            const seg = {
                ...
                oneWay: function oneWay(ball, hitWorld) {
                    ...
                    const hitDirection = configuredDirection(el, hitWorld || world);
                    ...
                    const hitLimits = gateSwingLimits(el, restAngle, hitDirection);
                    const swingDirection = swingDirectionForPush(hitDirection, pushDirection, hitLimits);
                    const passable = canSwing(gateState, swingDirection, hitLimits);
                ...
                onHit: function onHit(ball, hit, hitWorld) {
                    if (!isLocked(el, hitWorld || world) && !isOpen(el, hitWorld || world) && world && Pin.elements.getState) {
                        ...
                        const swingDirection = swingDirectionForPush(hitDirection, pushDirection, hitLimits);
                        const speed = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
                        if (directionAllows(el, hitWorld || world, pushDirection) && canSwing(state, swingDirection, hitLimits)) {
                            applyGateImpulse(el, state, speed, swingDirection, hitLimits);
                        } else {
                            state.angularVelocity = 0;
                            state.angle = clamp(state.angle || 0, hitLimits.min, hitLimits.max);
                        }
                    }
                    ball.vx *= 0.72;
                    ball.vy *= 0.72;
        ...
        editor: { handles: true, hitTest: true, inspectorFields: ["x", "y", "length", "angle", "direction", "open", "locked", "swingEndAngle", "returnStrength", "returnDamping", "thickness", "restitution", "color", "pinColor"] }
    });
})(window.Pin);
